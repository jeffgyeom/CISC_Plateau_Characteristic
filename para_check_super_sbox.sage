...
def GO_PROCESS(LOCK, PROC_NUM, SHARED_INFO):
    DONE_FLAG = False
    ...
    while True:
        ...
        with open("%d_rst_file.pickle"%(pidx), "wb") as f:
            pickle.dump(valid_trails_dict, f)
            pickle.dump(ana_rst_tab, f)

class SuperSboxPara:
    def __init__(self, sbox, field, matrix):
        global LOAD_VECS
    ...
    def analysis_plateau_char(self):
        ...
        for proc_idx in range(NUM_PROCESS_THIS_SYS):
            PROCESSES.append(mt.Process(target=GO_PROCESS, args=(LOCK, proc_idx, SHARED_INFO,)))
        ...
        for pidx in range(NUM_WORKLOADS):
            with open("%d_rst_file.pickle"%(pidx), "rb") as f:
...
if __name__ == "__main__":
    ...
    for sidx in range(len(SBOXES)):
        ...
        valid_trails_dict = S_PARA.valid_trails_dict
